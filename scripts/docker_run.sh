...
READY_MSG="${READY_MSG:-机械臂已加载|Kuka\+Allegro 已加载|Allegro 灵巧手已加载|Shadow Hand 已加载|Wuji Hand 2 已加载|双 Kuka\+Allegro 已加载|桌面张开姿态|双臂场景已加载|演示场景已加载}"
...
for round in $(seq 1 "$MAX_ROUNDS"); do
  ...
  elif echo "$logs" | grep -q "Shadow Hand 已加载"; then
    stage="Shadow Hand 已就绪"
  elif echo "$logs" | grep -q "Wuji Hand 2 已加载"; then
    stage="Wuji Hand 2 已就绪"
  fi
